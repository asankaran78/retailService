...
import com.myRetail.retailService.controller.RetailServiceController;
...
public class RetailServiceTest {
    ...
    @Before
    public void setup() {
        retailService = new RetailService(productDetailsDelegate, productInfoRepository);
    }


    @Test
    public void getProductDetailsHappyPath()  {

        ResponseEntity<String> redSkyResponse = new ResponseEntity<String>(responseBody(),HttpStatus.OK);
        when(productDetailsDelegate.GetProductDetails(Mockito.any())).thenReturn(redSkyResponse);
        ...
        Product product = new Product("13860428", "test", productInfo);
        when(productInfoRepository.findByProductId("13860428")).thenReturn(product);


        ProductResponse productResponse=retailService.GetProductDetails("13860428");

        assertEquals(productResponse.getProductId(),"13860428");


    }

    @Test(expected=NotFoundException.class)
    public void getProductDetailsProductNotFoundDataStore()  {

        ResponseEntity<String> redSkyResponse = new ResponseEntity<String>(responseBody(),HttpStatus.OK);
        when(productDetailsDelegate.GetProductDetails(Mockito.any())).thenReturn(redSkyResponse);
        when(productInfoRepository.findByProductId("138604")).thenReturn(null);


        ProductResponse productResponse=retailService.GetProductDetails("138604");

    }

    @Test(expected=Exception.class)
    public void getProductDetailsProductNotFoundRedSky()  {

        when(productDetailsDelegate.GetProductDetails(Mockito.any())).thenThrow(HttpClientErrorException.class);
